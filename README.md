...
* 注：其实我们拷贝的并不是内核页表的全体。实际上，在内核中新申请一个变量、申请一个物理页面，都是需要物理地址的（比如“**p->trapframe = kalloc()**”，这里的“**p->trapframe**”表示的地址（无论虚拟、物理，由于恒等映射，一样的），并不会出现在我们拷贝的内容中）。因此，我们拷贝的不是内核页表的全体。
