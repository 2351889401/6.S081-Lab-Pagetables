## 页表的实验是目前为止做过的最难的一个实验。因为当时对“kernel/vm.c”和“kernel/proc.c”中的函数不熟悉，并且当时对内存管理单元（MMU）、多级页表查询物理地址的流程等理解的不深刻，所以当时做实验实际上跨度大概两周左右，中间好像经历过不少次失败，后来成功完成实验应该是有运气的成分。直到现在，我再看当时写的这份实验的核心代码，仍然感觉做的很不错。  
...
其中，红色方框的内容表示3级页表查询中9位的结果，“**..**”的个数表示了这是第几级页表，将它们组合可以得到完整的虚拟地址；后面的 **pte** 和 **pa** 分别是页表项和真实的物理地址，可以认为它俩差不多，因为 **pa** = （**pte** >> 10）<< 12，**pte**的后10位表示的是权限相关的内容。  
...
## 2. 整体的实验目标：模仿Linux内核，让每个进程的页表包括 user space 和 kernel space 的内存映射。这样的话，当在用户空间和内核空间切换时，就可以直接使用进程的页表，而不用像xv6内核那样切换用户页表和内核页表了。  
...
* 注：其实我们拷贝的并不是内核页表的全体。实际上，在内核中新申请一个变量、申请一个物理页面，都是需要物理地址的（比如“**p->trapframe = (struct trapframe *)kalloc()**”，这里的“**p->trapframe**”表示的地址（无论虚拟、物理，由于恒等映射，一样的），并不会出现在我们拷贝的内容中）。因此，我们拷贝的不是内核页表的全体。这也是为什么下面的“**scheduler()**”函数“**如果没有进程执行的时候，使用内核页表**”（因为这种情况下不使用内核页表的话，在内核代码中执行的时候比如访问某些变量，会造成缺页异常，内核就崩溃了）。
...
if(p->state == RUNNABLE) {
    ...
    //当前进程要执行的时候 切换为它的新页表 这样就可以直接使用用户空间的虚拟地址了 在任务（1）中还没达到这个效果 但是完成了任务（2）就可以达到了
...
所以，这一步骤的核心是在新页表中记录下用户页表（**user pagetable**）的全部内容，与拷贝内核页表（**kernel pagetable**）的区别在于拷贝的内核页表的内容不变，用户页表可能会动态变化
...
|------    |------     |------|
|无        |无         |continue | 
|无        |有        |通过前面定义的“**free_copy_kernel_pagetable()**”函数（递归的）释放新页表的页表项  
...
如果是3级查询，可能存在这样的情况：原来用户页表和新页表指向相同的物理地址，但是释放了那一块内存，并且申请了新的物理内存，让用户页表的虚拟地址指向新的物理地址（因为原始的xv6内核是正确的，就是说每当用户空间的映射改变时，用户页表都能及时的更新）。这时，用户页表记录的是对的，而新页表还没有更改，必须和用户页表一致。
