...
void vmprint(pagetable_t pagetable)
{
  ...
  pte_t pte;
  for(i=0; i<512; i++) {
    // if(i>=3) break; //暂时看下第一个进程的user mappings和它的copy_kernel_pagetable的映射能不能对得上
    pte = pagetable[i];
    if(pte & PTE_V) {
      int j;
...
void update_pagetable_3(pagetable_t new_pagetable, pagetable_t old_pagetable, int level, uint64 preva) {
  int i;
  for(i=0; i<512; i++) {
    uint64 va = (i << (12 + 9 * (3-level))) | preva;
    if(va >= LIMIT) break;
    // if(level == 3 && i<3) {
    //   printf("i = %d level = %d va=%p\n", i, level, va);
    //   printf("old_pagetable: %p\n", old_pagetable);
    // }
    pte_t new_pte = new_pagetable[i];
    pte_t old_pte = old_pagetable[i];
    if((old_pte & PTE_V) == 0) {
      if((new_pte & PTE_V) == 0) continue;
      else {
